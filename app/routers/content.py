...
COLLECTIONS = ["exams", "subjects", "topics", "courses", "lessons", "mock_tests", "current_affairs"]
...
@router.get("/exams")
def exams():
    return [clean(x) for x in get_db().exams.find({"is_published": True}).sort("order", 1)]
...
def questions(topic_id: str | None = None, subject_id: str | None = None, limit: int = 20):
    q = {"is_published": True}
    if topic_id: q["topic_id"] = topic_id
    if subject_id: q["subject_id"] = subject_id
...
def current_affairs():
    return [clean(x) for x in get_db().current_affairs.find({"is_published": True}).sort("date", -1).limit(50)]
...
def submit_test(test_id: str, payload: dict, user=Depends(current_user)):
    ...
    result = {"_id": uuid.uuid4().hex, "user_id": str(user["_id"]), "test_id": test_id,
              "score": correct, "total": total, "answers": answers, "created_at": datetime.utcnow()}
    db.test_results.insert_one(result)
    return {"score": correct, "total": total, "percentage": round((correct/total)*100, 2) if total else 0}
