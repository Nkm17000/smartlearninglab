...
def dashboard(user=Depends(admin_user)):
    """Return real MongoDB-backed admin dashboard metrics.

    The response keeps both a `counts` object and flat top-level fields so older
    and newer FE builds can consume the endpoint without showing false zeroes.
    """
    db = get_db()

    courses = db.courses.count_documents({})
    published_courses = db.courses.count_documents({"is_published": True})
    draft_courses = courses - published_courses
    ...
    # Different versions of the app have used different collection names for
    # quiz attempts. Support both without requiring a migration.
    quiz_attempts = 0
    for collection_name in ("quiz_attempts", "quiz_results", "results"):
        try:
            quiz_attempts += db[collection_name].count_documents({})
        except Exception:
            pass
