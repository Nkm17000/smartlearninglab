...
export const api={
 ...
 completeLesson:async id=>{try{return await request(`/lessons/${id}/complete`,{method:'POST'})}catch(e){const q=JSON.parse(await AsyncStorage.getItem(OFFLINE_QUEUE_KEY)||'[]');q.push({id:`lesson-${id}-${Date.now()}`,type:'complete_lesson',payload:{lesson_id:id},created_at:new Date().toISOString()});await AsyncStorage.setItem(OFFLINE_QUEUE_KEY,JSON.stringify(q));return {queued:true,lesson_id:id,message:'Saved offline. It will sync when you reconnect.'}}},
 ...
 aiCoach:()=>request('/ai/coach'),
 personalizedQuiz:b=>request('/ai/personalized-quiz',{method:'POST',body:JSON.stringify(b||{})}),
 studyPlan:b=>request('/ai/study-plan',{method:'POST',body:JSON.stringify(b||{})}),
 uploadPdfCourse:async(file)=>{const token=await AsyncStorage.getItem(TOKEN_KEY);const form=new FormData();form.append('file',file);const r=await fetch(`${BASE_URL}/admin/ai/course-from-pdf`,{method:'POST',headers:{Accept:'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},body:form});const raw=await r.text();let d=null;try{d=raw?JSON.parse(raw):null}catch{d=raw}if(!r.ok)throw new Error(d?.detail||d?.message||`Request failed (${r.status})`);return d},
 savePdfCourse:b=>request('/admin/ai/course-from-pdf/save',{method:'POST',body:JSON.stringify(b)}),
 atRiskStudents:()=>request('/admin/students/at-risk'),
 careerRoadmap:(role='AI Engineer')=>request(`/career/roadmap?role=${encodeURIComponent(role)}`),
 mockInterview:b=>request('/ai/mock-interview',{method:'POST',body:JSON.stringify(b||{})}),
 evaluateMockInterview:b=>request('/ai/mock-interview/evaluate',{method:'POST',body:JSON.stringify(b||{})}),
 courseHealth:id=>request(`/admin/courses/${id}/health`),
 globalSearch:(q,limit=20)=>request(`/search?q=${encodeURIComponent(q)}&limit=${limit}`),
 syncOffline:async()=>{const q=JSON.parse(await AsyncStorage.getItem(OFFLINE_QUEUE_KEY)||'[]');if(!q.length)return {synced:[],failed:[],remaining:0};try{const r=await request('/offline/sync',{method:'POST',body:JSON.stringify({actions:q})});const failed=r.failed||[];const failedIds=new Set(failed.map(x=>x.id));const remaining=q.filter(x=>failedIds.has(x.id));await AsyncStorage.setItem(OFFLINE_QUEUE_KEY,JSON.stringify(remaining));return {...r,remaining:remaining.length}}catch(e){return {synced:[],failed:[],remaining:q.length,offline:true,message:e.message}},
